...
def main():
    ...
    version_bumped = 'version: "1.1.5"' in config_text
    ...
    if has_budget_provider and has_budget_model and version_bumped and session_token_budget_null and has_comments and state_file_written:
        print_ok("Upgrade successfully migrated configurations, bumped version to 1.1.5, kept comments, and wrote state file.")
        ...
        print_err(f"Upgrade verification failed! Injections:\n"
                  ...
                  f"  version: 1.1.5: {version_bumped}\n"
    ...
    res = run_command([sys.executable, "bootstrap/upgrade.py", "--project-path", str(TEST_PROJECT), "--force"])
    sidecar_exists = (TEST_PROJECT / ".agent" / "governance.md.framework-v1.1.5").exists()
    gov_preserved = gov_file.read_text(encoding="utf-8") == "Modified governance contents!"
    
    if sidecar_exists and gov_preserved:
        print_ok("Conflict trigger successfully detected modifications, wrote framework-v1.1.5 sidecar, and preserved original file.")
    else:
    ...
    c_content = re.sub(r'version: "1.1.5"', 'version: "1.1.0"', c_content)
    config_file.write_text(c_content, encoding="utf-8")
    
    # 3. Run upgrade again, check if it triggers re-verify mode
    res = run_command([sys.executable, "bootstrap/upgrade.py", "--project-path", str(TEST_PROJECT), "--force"])
    if "Project is already at version 1.1.5. Entering non-destructive verification pass." in res.stdout:
    ...
    crashing_migration_file = WORKSPACE_ROOT / "bootstrap" / "migrations" / "v1_1_4_to_v1_1_5.py"
...
"""
    ...
    # Run upgrade first to prepare the framework-v1.1.5 state
    run_command([sys.executable, "bootstrap/upgrade.py", "--project-path", str(TEST_PROJECT), "--force"])
    ...
    # Upgrade project to 1.1.5 cleanly
    setup_fresh_v110_project()
    run_command([sys.executable, "bootstrap/upgrade.py", "--project-path", str(TEST_PROJECT), "--force"])
    
    # Create the files expected by validate.py in their final 1.1.5 state
    ...
    (TEST_PROJECT / ".agent" / "UNIVERSAL_CONTEXT.md").write_text("Framework version: 1.1.5", encoding="utf-8")
