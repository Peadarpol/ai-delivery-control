...
5. **MUST update `.agent/state/decisions_log.md`** — document all technical, design, and business decisions made during this session. **Discipline**: Never edit `.agent/state/decisions_log.md` directly with a file-write tool. Always call `record_decision(title, decision, context, consequence)` from `harness_utils.py`. This guarantees append-only ordering and a consistent entry format. Direct edits to this file are a governance violation equivalent to editing `harness_events.jsonl` by hand. **Archival check**: Call `archive_old_decisions()` from `harness_utils.py` after calling `record_decision()` at session close. This automatically archives the oldest entries to `.agent/state/decisions_log_archive.md` if `decisions_log.md` exceeds 150 lines, and refuses to run if the log is not in ascending chronological order (in which case, stop and report — do not manually move entries). Never move entries between these files with a file-write tool directly.
