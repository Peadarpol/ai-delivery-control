...
- **Phase D — Producer/consumer contract, tooling-staleness, doc/code drift, and coverage-completeness checks.** Four related but distinct mechanisms, each targeting one of the shapes named in §1(4):
  ...
  - **D2 (Tooling-Path Staleness Checks):** for internal tooling scripts under `.agent/scripts/` and `.agent/skills/**/scripts/`, a static check flagging any hardcoded path literal referencing another file within the same repo that does not currently resolve to a real file. Additionally: when such a script reports a "clean"/"zero findings" result, cross-check that its target files were confirmed to exist and be non-trivially scanned before that result is treated as trustworthy — directly targeting the exact failure signature found in LOOP-013 (`wiki_lint_findings.md` reporting "0 issues" while silently short-circuiting on a path that never resolved).
  - **D3 (Documented-Trigger/Code Drift Checks):** absorbs the item already named in §8 as deferred. Parses `.agent/workflows/*.md` for documented escalation-trigger statements (the existing semi-structured convention seen in `eval-pipeline.md`'s "Escalation Triggers" section is the starting format; formalizing this convention across all workflow files that use it is a precondition, tracked as an assumption in §3, not assumed to already exist uniformly). For each documented trigger with a synthesizable precondition, a test creates that precondition and asserts the implementing script's actual exit code/behavior matches what's documented.
  - **D4 (Coverage-Completeness / Orphaned-Consumer Checks):** two sub-checks. First, for artifact-producing scripts/functions, confirm at least one consumer reference exists anywhere in the codebase; flag zero-consumer cases as `ORPHANED-PRODUCER` rather than leaving them to be found by manual audit (as LOOP-003/012/015/018 were). Second, for any loop entry in the maintained Loop Inventory classified `VERIFIED-WORKING`, confirm at least one test file exercises the real producer-to-consumer path (not a hand-crafted fixture bypassing the producer, per D1's concern); flag any gap as `UNTESTED-BUT-WORKING` — a distinct risk category from broken, since LOOP-017 demonstrated a loop can be correctly wired today with zero protection against silent future drift.
...
- `[Pending: D3 requires a documented convention for "escalation trigger" statements in workflow files to be either already-consistent across .agent/workflows/*.md or formalized as part of this phase. Do not assume the existing eval-pipeline.md format is already used uniformly elsewhere — audit this as the first step of D3, before building the parser, the same way Phase A first confirmed which specs already use Gherkin format.]`
...
### Scenario 9: Doc/code drift check catches the regression_runner.py discrepancy retroactively (D3)
...
### Scenario 10: Coverage-completeness check flags an untested-but-working loop (D4)
...
Then it flags this loop as `UNTESTED-BUT-WORKING`, distinct from `BROKEN`, surfacing the gap without requiring a human to have manually searched the test suite for its absence.
...
**Phase D — Producer/consumer contracts, tooling staleness, doc/code drift, coverage completeness** (retroactively justified by the loop-inventory audit; each sub-mechanism independently shippable)
- D1: seed the producer/consumer text-artifact registry; build the contract-test runner
- D2: static path-literal staleness scanner for internal tooling scripts; "clean result" cross-check
- D3: first audit which workflow files already use a consistent escalation-trigger format (precondition per §3); then build the parser and drift-check
- D4: orphaned-producer scanner; coverage-completeness cross-check against the maintained Loop Inventory
...
- **Status**: this is the original, coherent core of T1-K-19 — the three phases the spec was first drafted around. Phase A's matching-heuristic design must follow the algorithm specification added to §5 (Phase A) above before implementation begins.

**Tier 4 — Phase D** (follow-on, deferred by default)

- **Contents**: D1 (producer/parser contract tests), D2 (tooling-path staleness checks), D3 (documented-trigger/code drift checks), D4 (coverage-completeness/orphaned-consumer checks).
- **Delivery criteria**: Scenarios 7, 8, 9, 10 pass.
...
| New: `.agent/scripts/escalation_trigger_drift_check.py` | D3 — parses documented triggers, synthesizes preconditions, checks actual exit-code behavior | D |
| New: `.agent/scripts/coverage_completeness_check.py` | D4 — orphaned-producer scan + untested-but-working cross-check against Loop Inventory | D |
...
- **D4's "untested-but-working" flag is not itself a defect report.** A loop can be correctly wired and simply not yet warrant a dedicated test given its risk profile — D4 surfaces the gap for a human triage decision, the same way Phase A/B's findings are advisory, not auto-failing.
