...
- **Delivery criteria**: Scenarios 7, 8, and 10 (D1, D2, D4) pass. Scenario 9 (D3 retroactive check) is deferred alongside D3 implementation pending D3-scoping audit results.
...
- **D4's input list — `LOOP_INVENTORY.md` — is itself hand-maintained with no mechanism keeping it current.** D4's coverage is therefore bounded by a document that can silently go stale as loops are added, fixed, or retired without a corresponding inventory update. This is the same failure shape as LOOP-013's stale hardcoded paths, one level up: a checking mechanism whose own inputs can rot without signal. §3 resolves what the inventory is; nothing currently addresses that it can drift. Mitigate the same way Phase B's consumer-list drift is mitigated — by adding "update LOOP_INVENTORY.md" as a checklist item when a spec introduces or closes a loop — and treat D4's output as bounded by the inventory's own freshness, not as a complete census of loops.
