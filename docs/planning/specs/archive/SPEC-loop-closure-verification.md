...
- v1.19 (2026-08-07, Claude): Closes Scenario 6, found unbuilt by a pre-merge verification pass despite §5.5 recording Tier 3 as complete — `tests/helpers/outcome_equivalence.py` (reusable pattern), `tests/data/schema_hardening_fixture/` (hermetic fixture), and `tests/integration/test_outcome_equivalence.py` (retroactive buggy-refactor case plus forward correct-refactor case) now exist and pass. Also corrects two confirmed §6 Implementation Map path errors surfaced by the same pass: the Phase B script is `wiring_audit_core.py`, not `wiring_audit.py` (no file of the latter name has ever existed), and `wiring_consumers.yaml` lives at `.agent/scripts/`, not `.agent/config/`. The same stale script name in FRAMEWORK_ROADMAP.md is corrected in the same edit.
...
- **Phase D — Producer/consumer contract, tooling-staleness, doc/code drift, and coverage-completeness checks.** Four related but distinct mechanisms (three implementable in Tier 4; the fourth, D3, split into a preliminary scoping deliverable — see below):
  ...
  - **D4 (Coverage-Completeness / Orphaned-Consumer Checks):** originally scoped as two automated sub-checks; now one automated check plus one documented manual process, following empirical testing (see v1.17 changelog). D4a (retired as code, replaced by process): an AST-based orphaned-producer scan was built and tested against the real LOOP_INVENTORY.md — it flagged every genuinely-wired file-coupled loop as orphaned, because this codebase's actual producer/consumer coupling is almost entirely file-based (a producer writes a path, a consumer independently reads the same path), not Python-level imports, which is the only coupling shape AST reference search can detect. The real failure modes this spec's own audits found (HIB-080, LOOP-001, LOOP-004, LOOP-013) are all this file/schema-based category — meaning D4a's code-based approach was structurally aimed at a failure mode that essentially doesn't occur here. Retired in favor of `.agent/workflows/loop-audit.md`, a documented per-loop checklist an agent runs directly, mirroring the methodology LOOP_INVENTORY.md was already built with. D4b (delivered, unaffected): for any loop entry in the maintained Loop Inventory classified VERIFIED-WORKING, confirm at least one test file references both the named producer and consumer; flag any gap as `NO-COLOCATED-TEST-FOUND`. This check is agnostic to coupling mechanism — a test exercising a file-coupled pair still has to import both scripts to run them — so it doesn't inherit D4a's blind spot. Verified against three ground-truth cases (LOOP-002, LOOP-016, LOOP-017), including a genuine true negative.
...
### Scenario 11 (retired — see v1.17 changelog and §5.5 Tier 4): Orphaned-producer scan flags a real gap without false-positiving on a working pair (D4a)
...
**D4a — Retired, replaced by `.agent/workflows/loop-audit.md`** (see v1.17 changelog for the empirical finding). No script implements this check. Future loop-inventory maintenance runs the documented per-loop process instead.
...
**D3-Scoping (Preliminary Deliverable - Delivered)**:
Audited all 18 `.agent/workflows/*.md` files against `eval-pipeline.md`'s existing escalation-trigger convention. The audit confirmed that D3 is a **documentation-standardisation project across many files**, not a small parser: only 1 of 18 files (`eval-pipeline.md` itself) matches the reference convention, 10 express escalation/blocking rules through 5 heterogeneous structural formats, and 7 carry no escalation content at all. Full findings, verbatim evidence, and methodology accounting are recorded in [D3-SCOPING-AUDIT.md](file:///c:/projects/ai-delivery-control/docs/planning/specs/archive/D3-SCOPING-AUDIT.md).
...
**Tier 4 — Phase D** (partial delivery, remaining items deferred)

- **Contents**: D1 (producer/parser contract tests), D2 (tooling-path staleness checks), D3-scoping (preliminary workflow audit; D3 implementation split out), D4b (coverage-completeness check). D4a is retired as an automated mechanism — see v1.17 changelog — and replaced by `.agent/workflows/loop-audit.md`, a documented process rather than a Tier 4 code deliverable.
- **Delivery criteria**: Scenarios 7, 8, 10, and 12 (D1, D2, D4b) pass. Scenario 9 (D3 retroactive check) is deferred alongside D3 implementation. Scenario 11 (D4a) is retired, not deferred — it will not be attempted as automated tooling; `.agent/workflows/loop-audit.md` covers its intended purpose by a different means, outside this spec's Tier 4 scope. D1's delivery criteria require both the retroactive (Scenario 7) and forward (Scenario 12) cases.
- **Status**: D1 (producer/parser contract test runner) and D2 (tooling-path staleness scanner) delivered and verified in release v1.4.15 (Scenarios 7, 8, 12 passing cleanly). D3-scoping audit completed and published ([D3-SCOPING-AUDIT.md](file:///c:/projects/ai-delivery-control/docs/planning/specs/archive/D3-SCOPING-AUDIT.md)); D3 implementation itself remains deferred for a confirmed reason (audit proved scope is a documentation-standardisation effort across 17 non-conforming workflow files, not a small parser). D4b (coverage-completeness check) delivered and verified in release v1.4.15 (Scenario 10 passing, including a genuine true negative against LOOP-017). D4a is retired, not pending — see v1.17 changelog for the empirical finding that motivated retirement, and `.agent/workflows/loop-audit.md` for its replacement.
...
| New: `.agent/scripts/wiring_audit_core.py` | AST-based multi-consumer reference checker | B |
| `.agent/scripts/wiring_audit_core.py` (same file, new function) | Schema/sanity validation of `wiring_consumers.yaml` on load — reject empty consumer lists, malformed entries; fail loud, not silent-pass | B |
| `.agent/scripts/wiring_consumers.yaml` (new) | Seeded list of shared artifacts → documented consumers, per §3 assumption | B |
...
| `.agent/scripts/coverage_completeness_check.py` | D4b only — NO-COLOCATED-TEST-FOUND cross-check against Loop Inventory. D4a's orphaned-producer function is retained in the file (not deleted, since it's harmless dead code and may inform `.agent/workflows/loop-audit.md`'s design) but is not part of this spec's delivered scope and its output should not be treated as signal. | D |
| New: `.agent/workflows/loop-audit.md` | D4a's replacement — documented per-loop manual audit process | D |
...
- **D4a was built, tested against the real LOOP_INVENTORY.md, and retired after the test itself revealed a design flaw, not an implementation bug.** AST-based reference search can only detect Python-level code coupling (imports, function calls). Every confirmed real defect this spec's audits ever found — HIB-080, LOOP-001, LOOP-004, LOOP-013 — was file-based or schema-based drift, a category D4a's mechanism cannot see by construction. Run against the full inventory, it flagged 9 of 9 real pairs as orphaned, independently confirmed as a false-positive rate via direct trace of LOOP-001 (verified zero Python coupling between `distill_dream.py` and `harness_health.py`, yet genuinely wired via shared-file access). This is a stronger finding than "the heuristic needs tuning" (the Phase A word-overlap pattern) — D4a was structurally aimed at a failure mode this codebase doesn't exhibit. Worth remembering as a category distinct from Phase A's calibration story: not every mechanical check that seems reasonable in the abstract is actually well-matched to a given codebase's real coupling patterns, and testing against real ground truth before trusting a design — which is what caught this — is the mitigation, not a tuning parameter.
...
- **D4a (automated orphaned-producer scan) is not deferred, it is retired** — distinct from every other item in this section, which remains a candidate for future work. D4a's underlying purpose (finding producers with no consumer) is instead served by `.agent/workflows/loop-audit.md`'s manual process, which is not automatable by the mechanism D4a attempted, given this codebase's actual coupling patterns.

---

Per standing protocol: this spec is APPROVED. Tiers 1, 2, and 3 are delivered and independently verified (see v1.12–v1.13 changelog and decisions_log.md) and are cleared to merge to main. Tier 4 (Phase D) is partially delivered: D1 (producer/consumer contract test runner) and D2 (tooling-path staleness scanner) have been executed and verified (Scenarios 7, 8, and 12 passing — see v1.16 changelog), and D3-scoping's audit is complete, concluding that D3 implementation itself should remain deferred for a confirmed reason (documentation-standardisation work across 17 non-conforming files, not a small parser — see D3-SCOPING-AUDIT.md). D4b (coverage-completeness check) has been executed and verified (Scenario 10 passing). D4a (orphaned-producer scan) was attempted, tested against the real Loop Inventory, and retired — see v1.17 changelog — in favor of `.agent/workflows/loop-audit.md`.
