...
- v1.5 (Gemini, gate advisories resolution, 2026-07-23): addressed 8 advisories from Pass 2 quality review. Expanded §2 Out of Scope to explicitly exclude CI/CD pipeline, upgrade flows, credential rotation, IDE plugins, monorepos, and binary packaging. Added 4 explicit assumptions in §3 for git availability, file:// clone URIs, venv env vars, and pre-commit pre-installation. Enforced hard upper-bound timeout (must not exceed 5.0 seconds) and defined exact diagnostic card schema in §4 Scenario 2. Defined concrete postcondition for sandbox removal ('path does not exist on filesystem and all children unlinked') and sequence guarantee for sandbox recreation in §4 Scenario 3. Specified derivation rule for [PROJECT_SRC_PATH] in §4 Scenario 4. Confirmed Python floor split resolution in §7.
...
* **Out of Scope**:
  ...
  - CI/CD pipeline configuration changes beyond pre-commit hook scaffolding.
  - Downstream harness update/upgrade flows or automated migration pipelines.
  - Credential storage, secret manager integration, or automated key rotation services.
  - IDE plugin compatibility or extension-level onboarding guides.
  - Multi-project monorepo layout management or workspace multi-root coordination.
  - Windows-specific binary installer packaging (`.exe` or `.msi`).
...
* `[Resolved: 'git' is available and executable on PATH across all developer and CI environments.]`
* `[Resolved: Local file clone URIs ('file://') are supported by git across all execution contexts including Windows UNC paths and WSL.]`
* `[Resolved: Virtual environment indicators ('CONDA_DEFAULT_ENV', 'CONDA_PREFIX', 'VIRTUAL_ENV') are reliably populated by standard Python venv, virtualenv, and Conda distributions.]`
* `[Resolved: The target environment has 'pre-commit' installed into its active virtualenv prior to validate.py running.]`
...
* **Then** the validator fails the runnability check within a bounded time (hard socket/HTTP timeout that MUST NOT exceed 5.0 seconds — a blocked corporate firewall or unresponsive endpoint must not hang the installer indefinitely)
* **And** outputs precise key location and authorization troubleshooting cards to stderr containing: (1) target provider name, (2) configured environment variable name, (3) resolution status or error category, and (4) actionable remediation step
...
* **And**, on any exit path (success, failure, or exception), the ephemeral sandbox under `.agent/scratch/validate_sandbox/` is fully removed — defined by the postcondition that the sandbox path does not exist on the filesystem and all child files/directories are unlinked — including on Windows, where `git clone` marks internal object files read-only and a bare `shutil.rmtree()` raises `PermissionError: [WinError 5]`
...
* **And** a sandbox left over from a prior crashed run (orphaned directory) does not corrupt or get silently reused by the next validation run — prior orphan is removed before new sandbox directory is created.
...
* **And** when `[PROJECT_SRC_PATH]` (a user-controlled value derived from the target project's package directory name under the repository root, e.g. 'src' or the project module directory name) is substituted into the `exclude:` regex, any regex metacharacters in that value (e.g. a literal `.` in a source-root name) are escaped first — a naive string substitution would silently produce a *weaker* regex (a stray `.` matches any character) rather than an error, which defeats the exclusion this scenario exists to guarantee, without ever surfacing as a visible failure.
...
- Implement live API preflight key validation using a mock low-token query.
  - **Timeout:** every preflight network call carries an explicit timeout (socket/HTTP timeout that MUST NOT exceed 5.0 seconds) at the socket/HTTP client level. No preflight call may block indefinitely.
...
* **Python Floor Split**: Retain the existing 3.9 hard floor (`install.py`) and add a 3.10 soft-recommend (`validate.py`), matching the patched validator capability (Changelog v1.2).
