...
`status` (`"ACTIVE"` | `"COMPLETED"`), `agent` (string), `model` (string), `cost_tier` (`"frontier"` | `"standard"` | `"local"` | `"unknown"`), `session_kind` (`"code"` | `"analysis"` | `"planning"` | `"review"`, defaults to `"code"`. Correctable at close time. Precedence: `agent_session_close.json` -> `outcome_override` (in `session.json`) -> start-time value -> `"code"`), `task_magnitude`
