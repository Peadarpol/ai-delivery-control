...
| v1.4.11 | 2026-07-24 | **Installer & validator onboarding hardening (SPEC-v1.4.11)**: target-repo self-installation guard (`F-COLD-1`, `is_harness_repo()`); ephemeral git-sandbox dry-run validator with cross-platform URI, bounded timeouts, interrupt safety, and Windows read-only teardown; live API key preflight (Anthropic/OpenAI `/v1/models`, ≤ 5 s timeout, credential scrubbing); regex-escaped source paths in pre-commit exclusions (`re.escape()`, F7); `--skip-validation` CLI flag; `bootstrap/common.py` shared helpers; 652 checksum files. |
